...
async function getWorkspaceTree() {
  const workspaceExec = await exec(`hyprctl workspaces -j | jq 'map({name: .name, id: .id, windows: .windows})'`)
...
wss.on('connection', function connection(ws) {
  ...
  ws.on('message', async function message(data) {
    ...
    if (json.action === "command") {
      ...
      const tree = await getWorkspaceTree()
      const active_workspace = await exec(`hyprctl -j activeworkspace | jq -r .name`);
      wss.clients.forEach(function each(client) {
        client.send(JSON.stringify({ action: "active_workspace_updated", response: active_workspace.stdout }));
        client.send(JSON.stringify({ action: "get_workspace_tree", response: tree }));
      });
    } else if (json.action === "active_todos_updated") {
      const contents = await exec(`cat ~/obsidian/todo/Active\\ todo.md`)
      wss.clients.forEach(function each(client) {
        client.send(JSON.stringify({ action: "active_todos_updated", response: contents.stdout }));
      });
    } else if (json.action === "system_ideas_updated") {
      const contents = await exec(`cat ~/obsidian/todo/System\\ ideas.md`)
      wss.clients.forEach(function each(client) {
        client.send(JSON.stringify({ action: "system_ideas_updated", response: contents.stdout }));
      });
    } else if (json.action === "screenshots_updated") {
      const contents = await exec(`find ~/*.png`)
      wss.clients.forEach(function each(client) {
        client.send(JSON.stringify({ action: "screenshots_updated", response: contents.stdout }));
      });
    }
  });
